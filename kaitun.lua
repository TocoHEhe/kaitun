...
-- ========== CHẶN 3TN (AN TOÀN HƠN) ==========
local coreGui = game:GetService("CoreGui")

local function safeRemove3TN(inst)
    if inst and inst.Name == "3TN" then
        task.defer(function()
            pcall(function()
...
-- Kiếm tra nếu 3TN đã tồn tại
for _, child in ipairs(coreGui:GetChildren()) do
    safeRemove3TN(child)
end

-- Lắng nghe khi 3TN xuất hiện mới
coreGui.ChildAdded:Connect(safeRemove3TN)
coreGui.DescendantAdded:Connect(safeRemove3TN)
